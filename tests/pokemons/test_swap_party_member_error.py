...
def test_change_pokemons_that_dont_exist(api_client_user):
    ...
    assert response.data["entering_the_party"][0] == "pokemon to exchange does not exist"
...
def test_change_pokemons_that_do_not_belong_to_the_user(api_client_user, create_user):
    ...
    assert response.status_code == 400
    assert not serializer.is_valid()
    assert response.data["entering_the_party"][0] == "this pokemon is not in user storage"
    assert response.data["leaving_the_party"][0] == "this pokemon is not in user storage"
